...
class APIGetStateCustomerStatus extends BaseController
{
    ...
    {
        if (!isset($data['Quotation_ID']) && !isset($data['APP_ID'])) {
            // throw new Exception('Request Parameter');
            throw new Exception('Not found Data. Check Parameter [\'Quotation_ID\'] , [\'APP_ID\']', 1000);
        }

        $this->Quotation_ID = $data['Quotation_ID'] ?? null;
    ...
    public function Get_CustomerStatus(Request $request)
    {
        try {
            ...
            $MT_PROVINCE = MT_PROVINCE::with(['mt_district', 'mt_district.mt_sub_district'])
                ->where('PROVINCE_ID', 10)
                ->first();

            if ($this->Quotation_ID != '' || $this->Quotation_ID != null) {
                $return_data = $this->check_stateByQid();
            } else {
                // ฟังก์ชันสำหรับลูกค้ารายเก่าๆที่ไม่มีข้อมูล Quotation (ข้อมูลหาย)
                $return_data = $this->check_stateByAPP_ID();
            }
    ...
    function Check_IDQuotation()
    {
        $QT = QUOTATION::select('QUOTATION_ID', 'CUSTOMER_NAME', 'STATUS_ID')
            ->where('QUOTATION_ID', $this->Quotation_ID)
            ->count();

        if ($QT == 0) {
            throw new Exception('Not found Data', 1000);
        }
    }


    function check_stateByQid()
    {
        $return_data = new \stdClass();
        // dd($this->Quotation_ID);

        $this->Check_IDQuotation();

        $object_Guarantor = $this->Check_Guarantor();
        // $return_data->Guarantor = $object_Guarantor;
        // dd($return_data->Guarantor);


        // wait approve
        $QT_status27 = QUOTATION::where('QUOTATION_ID', $this->Quotation_ID)
            ->where('STATUS_ID', '27')
            ->count();

        $check = 0;
        if ($QT_status27 == 0) {
            $Stepapprove = APPLICATION::select('CHECKER_RESULT', 'CUSTOMER_NAME', 'APP_ID')
                ->where('QUOTATION_ID', $this->Quotation_ID)
                ->orderBy('APP_ID', 'DESC')
                ->get();
        ...
        //     ->orderBy('APP_ID', 'DESC')
        //     ->toSql();
        $product_KYC = PRODUCT::select('MT_CATEGORY.CATEGORY_NAME', 'MT_BRAND.BRAND_NAME', 'SERIES_NAME')
        ...
        $DownPrice = DB::table('dbo.QUOTATION')
            ->select('DOWN_SUM_AMT')
            ->where('QUOTATION_ID', $this->Quotation_ID)
            ->get();
        $return_data->DownPrice = $DownPrice[0]->DOWN_SUM_AMT;
    ...
    function Check_Guarantor()
    {
        ...
        $guar_QT = DB::table('dbo.PROSPECT_GUARANTOR')
            ...
            ->where('QUOTATION.QUOTATION_ID', isset($this->Quotation_ID) ? $this->Quotation_ID : null)
            ->get();
        ...
        $Guarantor_ID = DB::table('dbo.PROSPECT_GUARANTOR AS PG')
            ...
            ->where('PG.QUOTATION_ID', isset($this->Quotation_ID) ? $this->Quotation_ID : null)
        ...
        $object_Guarantor->QUOTATION_ID = isset($this->Quotation_ID) ? $this->Quotation_ID : null;
